    ...
    methods
        function [imagedata,im,im0,same_state,renderused] = getImageData(iw,index,method,mode,rolling,userender)
            ...
            if userender
                ...
                im  = iw.nc_read(index,'im');
                im0 = iw.im0;
                renderused = true;
            end

            if ~(renderused&&strcmp(method,'fcd'))
        ...
        function [imagedata,x,z,t] = imageParser(iw,index,varargin)
            %% Input
            ax      = [];
            method  = 'fcd';
            mode    = 'Image';
...
%                     caxis(ax,cmax)
    ...
    methods % Synthetic Schlieren functions
        function b              = ss_buoyancy_field(~,u,v,ds,rho_ref)
            ...
            rho0    = 999.8;        % [kg/m3] reference density
        ...
        function [u,v]          = ss_solver_fcd(iw,Iref,Idef,notRecursion)
            ...
            fcd = iw.ss_fcd;
            if ~isempty(fcd.mask)&&notRecursion
...
%             cu = gpuArray(cu);
        ...
        function ss_mask_fill(iw)
            %% Fills masked areas with a repeated background pattern from the image
    ...
    methods % Image loading (imagedatastore)
        ...
        function [im0,im] = handleMerged(iw,index)
            ...
            iw.im0  = switchReadMode(iw,iw.im0index);
            im0     = iw.im0;
    ...
    methods (Hidden) % Synthetic schlieren pattern generator
        function ss_pattern(~,lambdarec,varargin)
            ...
            if nargin<2
                lambdarec = 6; % recommended wavelength
            end
...
%             clf(h,'reset')
            ...
            if isSaving
                fname = sprintf('%slambda_%.2gpx.tiff','..\',lambdarec);%*pxpercma4/pxpercm));
                fprintf('Saving \n%s\n',fname)
    ...
    methods
        function state = nc_create(iw,reset)
            ...
            if ~isfile(fname)||reset
                ...
                nccreate(fname,'u','Dimensions',{'y',y,'x',x,'t',Inf},'FillValue',nan)
                nccreate(fname,'v','Dimensions',{'y',y,'x',x,'t',Inf},'FillValue',nan)
                nccreate(fname,'b','Dimensions',{'y',y,'x',x,'t',Inf},'FillValue',nan)

                nccreate(fname,'x','Dimensions',{'x',x})
                nccreate(fname,'z','Dimensions',{'z',y})
        ...
        function nc_render(iw,index,data,field)
            ...
            % resize 2D data to original dimensions
            if any(strcmp(field,{'im','u','v','b'}))
                if all(size(data)~=size(iw.im0))
                    data = imresize(data,size(iw.im0));
                end
            end
    ...
    methods
        ...
        function changeLabel(iw,to)
            oldlabel = iw.label;
            frontlabel = string(iw.date,'uuuu_MM_d\');
            if nargin<2
                to = 'untitled';
            end
